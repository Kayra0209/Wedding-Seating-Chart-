...
export const parseCSVFromUrl = (url: string): Promise<GuestGroup[]> => {
        ...
        const guests: GuestGroup[] = results.data
          .map((row: any, index: number) => {
            ...
            const findValue = (keys: string[], exclude: string[] = []) => {
              ...
              let key = rowKeys.find(k => keys.some(target => k.trim() === target));
              // Then try includes match, but skip excluded terms
              if (!key) {
                key = rowKeys.find(k => 
                  keys.some(target => k.trim().includes(target)) && 
                  !exclude.some(ex => k.trim().includes(ex))
                );
              }
